...
# Considering weighted models.
    ...
    def _plot_bestmodels_hist(models, dep_int=None):
        ...
        If depth is limited to 100 km, each depth cell is 2 km.

        Parameters
        ----------
        models : list of arrays
            Posterior models to plot.
        dep_int : ndarray, optional
            Depth intervals for interpolation. If None, default 0–100 km at 0.5 km steps.
        ...
        """
        # Set default interpolation/depth binning
        if dep_int is None:
            dep_int = np.linspace(0, 100, 201)  # 0.5 km steps
            depbins = np.linspace(0, 100, 101)  # 1 km bins
        else:
            interp = dep_int[1] - dep_int[0]
            maxdepth = np.ceil(dep_int.max())
            dep_int = np.arange(dep_int[0], dep_int[-1] + interp / 2, interp / 2)
            depbins = np.arange(0, maxdepth + 2 * interp, interp)

        # Replace NaNs and extract valid models
        models2 = ModelMatrix._replace_zvnoi_h(models)
        models2 = [model[~np.isnan(model)] for model in models2]

        # Get interface depths by cumsum of layer thicknesses (second half of model)
        yinterf = [np.cumsum(model[model.size // 2:-1]) for model in models2]
        yinterf = np.concatenate(yinterf)

        # Interpolated models and single-model representations
        ...
        # Flatten Vs values for histogram
        vss_flatten = vss_int.flatten()
        vsinterval = 0.025  # km/s

        vs_histmin = vs_round(vss_flatten.min()) - 2 * vsinterval
        vs_histmax = vs_round(vss_flatten.max()) + 3 * vsinterval
        vsbins = np.arange(vs_histmin, vs_histmax, vsinterval)

        # Set up figure and axes
        fig, axes = plt.subplots(1, 2, figsize=(5, 6.5),
                                 gridspec_kw={'width_ratios': [4, 1]},
                                 sharey=True)
        fig.subplots_adjust(wspace=0.05)

        # 2D histogram: Vs vs Depth
        data2d, xedges, yedges = np.histogram2d(
            vss_flatten, deps_int.flatten(),
            bins=(vsbins, depbins)
        )
        ...
        # Overlay single-model lines (only 'mode' used here)
        vs_mode, dep_mode = singlemodels['mode']
        axes[0].plot(vs_mode, dep_mode, color='white', lw=1, alpha=0.9, label='mode')
        axes[0].legend(loc='lower left')

        # Histogram of interface depths
        counts, bins_edges, _ = axes[1].hist(
            yinterf, bins=depbins, orientation='horizontal',
            color='lightgray', edgecolor='k', alpha=0.7
        )

        # Styling
        axes[0].set_ylabel('Depth in km')
        axes[0].set_xlabel('$V_S$ in km/s')
        axes[0].set_title(f'{len(models)} models')
        axes[0].invert_yaxis()
    ...
    def plot_posterior_nlayers(self, final=True, chainidx=0):
        ...
        # Remove NaNs from each model and convert to numpy array
        models = np.array([model[~np.isnan(model)] for model in models], dtype=object)

        # Compute number of layers: each layer has two parameters (v, z), minus the half-space
        layers = np.array([(model.size // 2) - 1 for model in models])

        # Histogram bins: from min-0.5 to max+0.5 to center bins on integers
        layer_min, layer_max = layers.min(), layers.max()
        bins = np.arange(layer_min, layer_max + 2) - 0.5  # +2 to include right edge

        formatter = '%d'
        ax = self._plot_posterior_distribution(layers, bins, formatter)

        xticks = np.arange(layer_min, layer_max + 1)
    ...
    def plot_posterior_models2d(self, final=True, chainidx=0, depint=1):
        ...
        # Create depth intervals for interpolation grid
        depth_grid = np.arange(
            self.priors['z'][0],
            self.priors['z'][1] + depint,
            depint
        )

        fig, axes = self._plot_bestmodels_hist(models, depth_grid)

        # Adjust plot appearance
        axes[0].set_ylim(self.priors['z'][::-1])  # depth increases downward
        axes[0].set_title(f'{len(models)} models from {nchains} chains')
