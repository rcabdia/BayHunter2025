...
# init model and misfit / likelihood
    ...
    def draw_initnoiseparams(self):
        ...
        for i, target in enumerate(self.targets.targets):
            for j, noiseref in enumerate(noiserefs):
                ...
                # Modern NumPy: treat any scalar (Python or NumPy) as fixed
                if np.isscalar(noiseprior):
                    corrfix[idx] = True
    ...
    def draw_initvpvs(self):
        # Modern NumPy: np.float removed; use scalar check
        if np.isscalar(self.priors['vpvs']):
...
# run optimization
    ...
    def run_chain(self):
        ...
        # Modern NumPy: np.float removed; use scalar check
        self.vpvsmods = [] if np.isscalar(self.priors['vpvs']) else ['vpvs']
