...
class ModelMatrix(object):
    ...
    def get_singlemodels(models, dep_int=None, misfits=None):
        ...
        # (5) mode from histogram  (fixed for modern NumPy)
        vss_flatten = vss_int.ravel()
        deps_flatten = deps_int.ravel()

        # number of vs bins (~0.025 km/s step); ensure at least 1 bin
        vs_range = float(vss_flatten.max() - vss_flatten.min())
        vsbins = max(1, int(np.ceil(vs_range / 0.025)))

        # histogram2d returns (H, xedges, yedges)
        H, vs_edges, dep_edges = np.histogram2d(
            vss_flatten, deps_flatten, bins=(vsbins, dep_int)
        )

        # bin centers
        vs_center = 0.5 * (vs_edges[:-1] + vs_edges[1:])
        dep_center = 0.5 * (dep_edges[:-1] + dep_edges[1:])

        # H shape: (len(vs_edges)-1, len(dep_edges)-1) = (n_vs_bins, n_dep_bins)
        # for each depth bin (column), pick the vs bin index with the maximum count
        idx = np.argmax(H, axis=0)  # length = n_dep_bins
        vs_mode = vs_center[idx]
